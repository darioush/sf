...
Fixpoint nonzeros (l:natlist) : natlist :=
  match l with
  | nil => nil
  | h :: t => match h with | 0 => (nonzeros t) | S _ => h :: (nonzeros t) end end.
...
Fixpoint oddmembers (l:natlist) : natlist :=
  match l with
  | nil => nil
  | h :: t => match (oddb h) with | false => (oddmembers t) 
                                  | true => h :: (oddmembers t) end end.

Example test_oddmembers:            oddmembers [0;1;0;2;3;0;0] = [1;3].
Proof. reflexivity. Qed.

Fixpoint countoddmembers (l:natlist) : nat :=
  match l with | nil => 0 | h :: t => 
          match oddb h with | false => countoddmembers t
                            | true =>  S (countoddmembers t) end end.
...
Definition member (v:nat) (s:bag) : bool := 
  match count v s with | 0 => false | S _ => true end.
...
Fixpoint remove_all (v:nat) (s:bag) : bag :=
    match s with
    | nil => nil
    | h :: t => match beq_nat v h with | true => remove_all v t 
                                       | false => h :: remove_all v t end
    end.
...
Theorem beq_n_n: forall n: nat, beq_nat n n = true.
Proof.
    intros n. induction n. reflexivity.
    simpl. rewrite -> IHn. reflexivity.
Qed.

Theorem bag_count_add: forall b: bag, forall n: nat,
...
Theorem bag_count_concat: forall a b : bag , forall n: nat,
        count n a + count n b = count n (a ++ b).
Proof.
    intros a b n.
...
Fixpoint beq_natlist (l1 l2 : natlist) : bool :=
  ...
  | nil, _ => false
  | _, nil => false
  | h1::t1, h2::t2 => match beq_nat h1 h2 with | true => beq_natlist t1 t2
                                               | false => false end
  end.
...
Proof.
    ...
    simpl.
    destruct n. rewrite -> ble_n_Sn. reflexivity.
...
Proof.
    intros l1 l2.
    rewrite <- rev_involutive with (l := l1) at 2.
    rewrite <- rev_involutive with (l := l2) at 2.
    intros H.
    rewrite H.
    reflexivity.
Qed.
